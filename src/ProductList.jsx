...
function ProductList({ onHomeClick }) {
  ...
  const handleAddToCart = (product) => {
    dispatchEvent(addItem(product)); // Dispatch the addItem action with the product
    setAddedToCart((prev) => ({ ...prev, [product.name]: true })); // Update the addedTo
  };
  const handleRemoveToCart = (product) => {
    dispatchEvent(removeItem({ name: product.name })); // Dispatch the removeItem action with the product name
    setAddedToCart((prev) => ({ ...prev, [product.name]: false })); // Update the addedToCart state to false for the removed product
  };

  const calculateTotalQuantity = () => {
    //     return addedToCart.reduce((total, item) => total + item.quantity, 0); // Calculate total quantity of items in the cart
    //   };
    return CartItem
                  ...
                  {category.plants.map(
                    (
                      ...
                      <div className="product-card" key={plantIndex}>
                        ...
                        <button
                          className="product-button"
                          onClick={() => handleRemoveToCart(plant)} // Handle removing plant from cart
                        >
                          Remove from Cart
                        </button>
                        ...
                        {/* Show "Added to Cart" message if the plant is in the cart */}
                        <div className="product-quantity">
                          Quantity: {calculateTotalQuantity(plant.name)}{" "}
                          {/* Display the total quantity of the plant in the cart */}
                        </div>
                        {/* Display the total quantity of the plant in the cart */}
                      </div>
                    )
